...
docker exec -i bidorai-db psql -U postgres -c "CREATE DATABASE bidorai;"
docker exec -i bidorai-db psql -U postgres -d bidorai < database\init-db.sql
...
echo Starting frontend server in debug mode...
start cmd /k "npm run dev"
